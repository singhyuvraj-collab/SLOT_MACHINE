...
# this is to print the choosen symbols with their values
 ...
 # from here, there are different scenarios that are possible in slot machine
    ...
    if symbol_1 == "7" and symbol_2 == "7" and symbol_3 == "7":
        ...
        reward = value_1 + value_2
        print("your reward is: ", reward)
       
    else:
        reward = 0
        print("Try again. Dont give up ", NAME)

    if reward != 0:  # in case if the user is winning something, then his spin fee will be refunded
        MONEY = left_money + reward + 20
        total_winnings = total_winnings + reward
        print("your total winning is:", MONEY)

    if reward == 0:   # in case if the user is not winning something, then the spin fee will not be refunded
        MONEY = left_money
        total_winnings = total_winnings + reward
        print("OOPS! you didn't won any reward", MONEY)
